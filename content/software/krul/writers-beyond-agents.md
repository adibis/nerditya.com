...
The interesting part isn't only that there's a new category of writer. It's that this category's writes have a fundamentally different shape than an agent's, with real, favorable concurrency consequences the previous two articles didn't get to use.
